---
title: "4. Estimate vegetation properties from sentinel-2 imagery"
...
vignette: >
  ...
  %\VignetteIndexEntry{4. Estimate vegetation properties from sentinel-2 imagery}
...
Sentinel-2 images and `prosail` hybrid inversion. 
...
# installation of `preprocS2`

The R package [`preprocS2`](https://jbferet.gitlab.io/preprocs2/) is used to 
access Sentinel-2 data. 
`preprocS2` is a basic wrapper of different R packages allowing access to STAC 
catalogs, such as [`rstac`](https://brazil-data-cube.github.io/rstac/), 
[`CDSE`](https://zivankaraman.github.io/CDSE/) and 
[`rstactheia`](https://forge.inrae.fr/teledec/rstactheia).
It aims at downloading and preparing Sentinel-2 images for further processings 
using R packages such as `prosail`.

Please visit the webpage dedicated to [`preprocS2`](https://jbferet.gitlab.io/preprocs2/)
...
[Sentinel-hub](https://browser.dataspace.copernicus.eu/?zoom=12&lat=39.06065&lng=-2.13238&themeId=DEFAULT-THEME&visualizationUrl=U2FsdGVkX19ygPtduKVETQXB2nFxaScLJzXrryULgCj1wP1rpi10GT%2F7mVLRqdcAshmDfC8D6iE9Onke%2BQTsJJ90Hu35jFerk3GJcr0s4%2FDV0U3NFYNAl9mCthFjtELl&datasetId=S2_L2A_CDAS&fromTime=2021-05-13T00%3A00%3A00.000Z&toTime=2021-05-13T23%3A59%3A59.999Z&layerId=1_TRUE_COLOR&demSource3D=%22MAPZEN%22&cloudCoverage=30&dateMode=SINGLE) 
...
The default STAC collection is `sentinel-2-l2a` from the Microsoft Planetary 
computer catalog.
...
aoi_path <- 'barrax_aoi.GPKG'
sf::write_sf(obj = aoi, dsn = aoi_path, driver = 'GPKG')
...
list_files <- get_s2_raster(aoi_path = aoi_path, datetime = datetime, 
                            ...
                            site_name = 'barrax', options = options)
...
url <- "https://gitlab.com/jbferet/myshareddata/-/raw/master/prosail_vignette_illustrations/S2/s2_rgb.png"
knitr::include_graphics(url)
```
<!-- <p> -->
<!-- <img src="../man/figures/S2/s2_rgb.png" width="100%" height="100%" class="center"> -->
<!-- </p> -->
<center>
  Fig. 1. RGB representation of a sentinel-2 acquisition (L2A) over Barrax, 
  Spain (2021-05-13).
</center> 
...
srf <- get_srf_sensor(sensor_name)

# define parameters to estimate
parms_to_estimate <- c('lai', 'chl', 'ewt', 'lma', 'fcover', 'fapar')
...
hybrid_model <- train_prosail_inversion(parms_to_estimate = parms_to_estimate,
...
the input variable `nb_models` from `hybrid_model` (default = 10 models) is saved.
...
```{r, eval=TRUE, echo = FALSE, out.width="100%"}
url <- "https://gitlab.com/jbferet/myshareddata/-/raw/master/prosail_vignette_illustrations/S2/hybrid_inversion_barrax.png"
knitr::include_graphics(url)
```
<center>
  Fig. 2. biophysical properties estimated from `prosail` hybrid inversion: 
  `chl`, `lai`, `fcover`, `fapar`.
</center> 
...
```{r, eval=TRUE, echo = FALSE, out.width="100%"}
url <- "https://gitlab.com/jbferet/myshareddata/-/raw/master/prosail_vignette_illustrations/Hybrid_Inversion/compare_SNAP_prosail.png"
knitr::include_graphics(url)
```
